...
export const data = new SlashCommandBuilder()
    ...
    .setRequired(true))
    .addStringOption(opt => opt.setName('tags')
    .setDescription('Thêm tag (cách nhau bằng dấu cách)')
    .setRequired(false))).addSubcommand(sub => sub.setName('delete')
...
export async function execute(interaction) {
    ...
    if (!match)
        return interaction.reply({ content: '❌ Link không hợp lệ.', ephemeral: true });

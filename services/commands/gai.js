...
export default {
        ...
        try {
            ...
            if (chatId) {
                ...
                try {
                    ...
                    await deleteGlobalChatById(userId, chatId);
    ...
    async showGlobalChatList(message) {
        try {
            const chatList = await getGlobalChatList();
    ...
    async startNewGlobalChat(message) {
        ...
        try {
            ...
            await createNewGlobalChat(senderId);
    ...
    async continueGlobalChat(message, chatId) {
        ...
        try {
            ...
            await updateGlobalChatTime(targetChat.id);
            const messages = await getGlobalChatMessages(targetChat.id, 5);
    ...
    async processGlobalChatMessage(message, args, model, sendEmbedMessage) {
        ...
        try {
            ...
            // Lấy lịch sử chat
            let historyRows = await getCurrentGlobalChatHistory(userId, 5);
            ...
            if (conversation.length === 0) {
                ...
                try {
                    ...
                    await addGlobalChatMessage(userId, 'user', prompt, userName);
                    await addGlobalChatMessage(userId, 'model', content, userName);
                    
                    // Tóm tắt và cập nhật tiêu đề cuộc trò chuyện
                    await summarizeAndUpdateGlobalChatTitle(userId, model);
